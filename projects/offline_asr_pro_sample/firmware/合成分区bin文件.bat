...
echo make asr.bin
for %%i in (asr\*) do (
	echo %%i | %SDK_PATH%\tools\build-tools\bin\busybox grep -E "\[.*\].*" > nul && set valid_file=%%i
	if "!valid_file!" neq "" (
		...
		if  "!head_flag!" neq "ver-1000" (
			...
			cscript !TOOLS_PATH!\message_box.vbs "分区文件合成失败!LF!文件%%i格式错误, !LF!请使用pro版本的asr模型和dnn模型。" "合成分区bin文件"
			goto:eof
		)
	)
)
...
copy %TOOLS_PATH%\..\libs\libfbin_pro.a user_code\[1]code.bin
...
echo make dnn
for %%k in (dnn\*) do (
	echo %%k | %SDK_PATH%\tools\build-tools\bin\busybox grep -E "\[.*\].*" > nul && set valid_file2=%%k
	if "!valid_file2!" neq "" (
		for /f %%j in ('%SDK_PATH%\tools\build-tools\bin\busybox head -c 4 !valid_file2!') do set head_flag2=%%j
		if  "!head_flag2!" neq "MOD2" (
			if  "!head_flag2!" neq "MOD3" (
			if  "!head_flag2!" neq "MOD5" (
				for /f %%a in ('echo prompt $E ^| cmd') do set "ESC=%%a"
				echo !ESC![31m文件%%k格式错误,请检查是否使用了正确版本的模型文件!ESC![0m
				echo !ESC![31m!head_flag2!!ESC![0m
				set LF=^


				cscript !TOOLS_PATH!\message_box.vbs "分区文件合成失败!LF!文件%%k格式错误, !LF!请使用pro版本的asr模型和dnn模型。" "合成分区bin文件"
				goto:eof
			)
			)
		)
