    ...
    if (p_cmd_models_table)
    {
        if (p_model_header)
        {
            ...
            for (i = 0;i < cmd_file_header.model_number;i++)
            {
                ...
                max_command_number = p_model_header->command_number > max_command_number ?
                                        p_model_header->command_number : max_command_number;
                max_special_word_number = p_model_header->special_word_number > max_special_word_number ?
                                        p_model_header->special_word_number: max_special_word_number;
                max_cmd_string_table_size = p_model_header->cmd_string_table_size > max_cmd_string_table_size ?
                                        p_model_header->cmd_string_table_size : max_cmd_string_table_size;
...
{
    return max_special_word_number;
}

uint32_t cmd_file_get_max_cmd_string_table_size()
{
    return max_cmd_string_table_size;
}
