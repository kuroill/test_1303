...
__STATIC_INLINE int decode_subframe(FLACContext *s, int channel, int32_t* decoded)
{
    int type, wasted = 0;
    int i, tmp;
...
static int decode_frame(FLACContext *s){
    ...
    /* subframes */
    if ((res=decode_subframe(s, 0, (int32_t*)s->decoded0)) < 0){
    	return res-100;
    }


    if (s->channels==2) {
      if ((res=decode_subframe(s, 1, (int32_t*)s->decoded1)) < 0){
    	  return res-200;
      }
    }
	...
	switch(fc->decorrelation)
	{
		case INDEPENDENT :
			if (fc->channels==1) 
			{
				do
				{
					//*(wavbuf ++) = *ch0;    //单声道不做复制操作 modify by QQM
					*(wavbuf ++) = *(ch0 ++);
				}while(-- sampleCnt);
			}else 
			{
