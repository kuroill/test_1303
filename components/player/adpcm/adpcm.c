...
#if !USE_MASK_ROM_PCM_TABLE_DATA
...
uint8_t ADPCM_Encode(int32_t sample)
{
    ...
    #else
    //use cmsis core_cmInstr.h modify by QQM
    Encodepredsample = __SSAT(Encodepredsample,16);
    #endif
...
int16_t ADPCM_Decode(uint8_t code)
{
    ...
    #else
    //use cmsis core_cmInstr.h modify by QQM
    Decodepredsample = __SSAT(Decodepredsample,16);
    #endif
...
{
    Decodeindex = decode_index;
}

void set_adpcm_decode_predsample(int32_t decode_predsample)
{
    Decodepredsample = decode_predsample;
}
