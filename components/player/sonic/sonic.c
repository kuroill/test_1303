...
int sonicSetSampleRate(sonicStream stream, int sampleRate) {
  freeStreamBuffers(stream);
  return allocateStreamBuffers(stream, sampleRate, stream->numChannels);
}
...
int sonicSetNumChannels(sonicStream stream, int numChannels) {
  freeStreamBuffers(stream);
  return allocateStreamBuffers(stream, stream->sampleRate, numChannels);
}

/* Clean a sonic stream.  */
...
static int addShortSamplesToInputBuffer(sonicStream stream, short* samples,
  ...
  MASK_ROM_LIB_FUNC->newlibcfunc.memcpy_p(stream->inputBuffer + stream->numInputSamples * stream->numChannels,
...
static void removeInputSamples(sonicStream stream, int position) {
  ...
  if (remainingSamples > 0) {
    MASK_ROM_LIB_FUNC->newlibcfunc.memmove_p(stream->inputBuffer,
...
static int copyToOutput(sonicStream stream, short* samples, int numSamples) {
  ...
  MASK_ROM_LIB_FUNC->newlibcfunc.memcpy_p(stream->outputBuffer + stream->numOutputSamples * stream->numChannels,
         samples, numSamples * sizeof(short) * stream->numChannels);
...
int sonicReadFloatFromStream(sonicStream stream, float* samples,
  ...
  if (remainingSamples > 0) {
    MASK_ROM_LIB_FUNC->newlibcfunc.memmove_p(stream->outputBuffer,
...
int sonicReadShortFromStream(sonicStream stream, short* samples,
  ...
  MASK_ROM_LIB_FUNC->newlibcfunc.memcpy_p(samples, stream->outputBuffer,
...
int sonicFlushStream(sonicStream stream) {
  ...
  MASK_ROM_LIB_FUNC->newlibcfunc.memset_p(stream->inputBuffer + remainingSamples * stream->numChannels, 0,
...
static int moveNewSamplesToPitchBuffer(sonicStream stream,
  ...
  MASK_ROM_LIB_FUNC->newlibcfunc.memcpy_p(stream->pitchBuffer + stream->numPitchSamples * numChannels,
...
static void removePitchSamples(sonicStream stream, int numSamples) {
  ...
  if (numSamples != stream->numPitchSamples) {
    MASK_ROM_LIB_FUNC->newlibcfunc.memmove_p(
...
static int adjustRate(sonicStream stream, float rate,
                      int originalNumOutputSamples) {
  int newSampleRate = (int)(stream->sampleRate / rate);
...
static int skipPitchPeriod(sonicStream stream, short* samples, float speed,
  ...
  if (speed >= 2.0f) {
    ...
    newSamples = period;
    stream->remainingInputToCopy = (int)(period * (2.0f - speed) / (speed - 1.0f));
  }
...
static int insertPitchPeriod(sonicStream stream, short* samples, float speed,
  ...
  if (speed < 0.5f) {
    ...
    stream->remainingInputToCopy =
        (int)(period * (2.0f * speed - 1.0f) / (1.0f - speed));
  }
  if (!enlargeOutputBufferIfNeeded(stream, period + newSamples)) {
    return 0;
  }
  out = stream->outputBuffer + stream->numOutputSamples * numChannels;
  MASK_ROM_LIB_FUNC->newlibcfunc.memcpy_p(out, samples, period * sizeof(short) * numChannels);
  out =
